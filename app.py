...
def build_technical_row(symbol, history, live_price=None):
    ...
    # ------------------------------------------------------------
    # BREAKOUT LEVELS: DAILY / WEEKLY / MONTHLY / NEAR ATH
    # ------------------------------------------------------------
    # Daily: previous 20 trading-session high.
    ...
    daily_breakout = (
        live_close > daily_breakout_level
        if daily_breakout_level is not None
        else False
    )

    # Weekly: aggregate daily OHLC into completed weekly candles.
    weekly_buckets = {}
    for ts, op, hi, lo, cl, vol in zip(
        ...
        try:
            ...
            bucket = weekly_buckets.setdefault(
                key,
                {"open": op, "high": hi, "low": lo, "close": cl, "volume": 0.0}
            )
    ...
    weekly_bars = [weekly_buckets[k] for k in sorted(weekly_buckets)]
    ...
    weekly_breakout = (
        live_close > weekly_breakout_level
        if weekly_breakout_level is not None
        else False
    )

    # Monthly: aggregate daily OHLC into completed monthly candles.
    monthly_buckets = {}
    for ts, op, hi, lo, cl, vol in zip(
        timestamps, opens, highs, lows, closes, volumes
    ):
    ...
    monthly_bars = [monthly_buckets[k] for k in sorted(monthly_buckets)]
    ...
    monthly_breakout = (
        live_close > monthly_breakout_level
        if monthly_breakout_level is not None
        else False
    )
    ...
    # Bullish price action on the latest daily candle.
    ...
    price_action_bullish = (
        ...
        and live_close > latest_open
    )
    strong_close = (
        latest_high is not None
        and latest_low is not None
        and latest_high > latest_low
        and ((live_close - latest_low) / (latest_high - latest_low)) >= 0.65
    )
    ...
    # Combined breakout buyer: any requested timeframe breakout +
    # bullish price action + strong volume.
    breakout_any = (
        daily_breakout
        or weekly_breakout
        or monthly_breakout
    ...
    # Preserve the original names used by the existing scanner.
